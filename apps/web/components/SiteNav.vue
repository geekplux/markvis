...
const GITHUB_REPO_API = "https://api.github.com/repos/geekplux/markvis";
const STAR_CACHE_KEY = "markvis-github-stars";
const STAR_CACHE_TTL_MS = 60 * 60 * 1000;

const starCount = ref<number | null>(null);

const starAriaLabel = computed(() => {
  if (starCount.value == null) return "Star MarkVis on GitHub";
  return `Star MarkVis on GitHub (${starCount.value} stars)`;
});

function formatStarCount(n: number): string {
  if (n < 1000) return String(n);
  const tenths = Math.round(n / 100) / 10;
  return `${tenths % 1 === 0 ? tenths.toFixed(0) : tenths}k`;
}
  ...
  try {
    ...
    if (raw) {
      ...
      if (
        ...
        Date.now() - parsed.at < STAR_CACHE_TTL_MS
      ) {
        starCount.value = parsed.count;
        return;
      }
    }
  } catch {
