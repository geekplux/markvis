...
| `theme` | no | `folio` | `folio` \| `highcharts` \| `shadcn` \| `docs` \| `ant` only. |
...
| `E_UNKNOWN_THEME` | `theme` not in `folio` \| `highcharts` \| `shadcn` \| `docs` \| `ant`. |

Unknown failures still degrade to table + one line; prefer a listed code when it fits.

## Themes

Optional fence header `theme:` selects a named look. Allowed values: `folio` (default), `highcharts`, `shadcn`, `docs`, `ant`. Omit the field to get `folio`. An unknown value is a stable parse error (`E_UNKNOWN_THEME`) with table fallback — never a silent default swap. Themes are IR metadata only in this unit; they do not add chart types or pull Highcharts/d3/Unovis.
